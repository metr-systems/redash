            ...
            <Menu mode="vertical" theme="dark" selectable={false} className="mobile-navbar-menu">
              {currentUser.hasPermission("list_dashboards") && (
                <Menu.Item key="dashboards">
                  <Link href="dashboards">{i18next.t("ApplicationArea:Dashboards")}</Link>
                </Menu.Item>
              )}
              {currentUser.hasPermission("view_query") && (
                <Menu.Item key="queries">
                  <Link href="queries">{i18next.t("ApplicationArea:Queries")}</Link>
                </Menu.Item>
              )}
              {currentUser.hasPermission("list_alerts") && (
                <Menu.Item key="alerts">
                  <Link href="alerts">{i18next.t("ApplicationArea:Alerts")}</Link>
                </Menu.Item>
              )}
              <Menu.Item key="profile">
                <Link href="users/me">{i18next.t("ApplicationArea:Edit Profile")}</Link>
              </Menu.Item>
              <Menu.Divider />
              {firstSettingsTab && (
                <Menu.Item key="settings">
                  <Link href={firstSettingsTab.path}>{i18next.t("ApplicationArea:Settings")}</Link>
                </Menu.Item>
              )}
              ...
              <Menu.Item key="help">
                ...
                <Link href="https://redash.io/help" target="_blank" rel="noopener">
                  {i18next.t("ApplicationArea:Help")}
                </Link>
              </Menu.Item>

...
function AddToDashboardDialog({ dialog, visualization }) {
    ...
    Dashboard.get(selectedDashboard)
      ...
      .then(dashboard => {
        ...
        notification.success(
          t("Queries:Widget added to dashboard"),
          <React.Fragment>
      ...
      .catch(() => {
        notification.error(t("Queries:Widget not added."));
      })
      .finally(() => {
  ...
  return (
    <Modal
      {...dialog.props}
      title={t("Queries:Add to Dashboard")}
      ...
      <label htmlFor="add-to-dashboard-dialog-dashboard">{t("Queries:Choose the dashboard to add this query to")}:</label>
      ...
      {!selectedDashboard && (
        <Input
          ...
          autoComplete="off"
          autoFocus
          placeholder={t("Queries:Search a dashboard by name")}
          value={searchTerm}

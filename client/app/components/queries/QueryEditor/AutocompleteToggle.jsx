...
export default function AutocompleteToggle({ available, enabled, onToggle }) {
  ...
  let tooltipMessage = t("Live Autocomplete Enabled");
  let icon = "icon-flash";
  if (!enabled) {
    tooltipMessage = t("Live Autocomplete Disabled");
    icon = "icon-flash-off";
  }

  if (!available) {
    tooltipMessage = t("Live Autocomplete Not Available (Use Ctrl+Space to Trigger)");
    icon = "icon-flash-off";
  }
  ...
  return (
    <Tooltip placement="top" title={tooltipMessage}>
      <Button
        ...
        disabled={!available}
        onClick={handleClick}
        aria-label={enabled ? t("Disable live autocomplete") : t("Enable live autocomplete")}>

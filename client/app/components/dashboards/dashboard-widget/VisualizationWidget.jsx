  ...
  return compact([
    <Menu.Item key="download_csv" disabled={isQueryResultEmpty}>
      {!isQueryResultEmpty ? (
        <Link href={downloadLink("csv")} download={downloadName("csv")} target="_self">
          {i18next.t("Dashboards:Download as CSV File")}
        </Link>
      ) : (
        i18next.t("Dashboards:Download as CSV File")
      )}
    ...
    <Menu.Item key="download_tsv" disabled={isQueryResultEmpty}>
      {!isQueryResultEmpty ? (
        <Link href={downloadLink("tsv")} download={downloadName("tsv")} target="_self">
          {i18next.t("Dashboards:Download as TSV File")}
        </Link>
      ) : (
        i18next.t("Dashboards:Download as TSV File")
      )}
    ...
    <Menu.Item key="download_excel" disabled={isQueryResultEmpty}>
      {!isQueryResultEmpty ? (
        <Link href={downloadLink("xlsx")} download={downloadName("xlsx")} target="_self">
          {i18next.t("Dashboards:Download as Excel File")}
        </Link>
      ) : (
        i18next.t("Dashboards:Download as Excel File")
      )}
    ...
    canViewQuery && (
      <Menu.Item key="view_query">
        <Link href={widget.getQuery().getUrl(true, widget.visualization.id)}>{i18next.t("Dashboards:View Query")}</Link>
      </Menu.Item>
    ),
    canEditParameters && (
      <Menu.Item key="edit_parameters" onClick={onParametersEdit}>
        {i18next.t("Dashboards:Edit Parameters")}
      </Menu.Item>
    ),
  ]);
}
        ...
        {!isPublic && !!widgetQueryResult && (
          <PlainButton
            ...
            <span className="sr-only">
              {refreshClickButtonId === 1 ? i18next.t("Dashboards:Refreshing, please wait. ") : i18next.t("Dashboards:Press to refresh. ")}
            </span>{" "}
      ...
      <span>
        {!isPublic && (
          <PlainButton
            ...
            <span className="sr-only">
              {refreshClickButtonId === 2 ? i18next.t("Dashboards:Refreshing, please wait.") : i18next.t("Dashboards:Press to refresh.")}
            </span>
          </PlainButton>
        )}

...
export default function QueryFormItem({ query, queryResult, onChange, editMode }) {
  ...
  const queryHint =
    query && query.schedule ? (
      ...
      <small>
        <WarningFilledIcon className="warning-icon-danger" /> {t("Alert:This query has no")} <i>{t("Alert:refresh schedule")}</i>.{" "}
        <Tooltip title={t("Alert:A query schedule is not necessary but is highly recommended for alerts. An Alert without a query schedule will only send notifications if a user in your organization manually executes this query.")}>
          <a role="presentation">
            {t("Alert:Why it&apos;s recommended")}<QuestionCircleTwoToneIcon />
          </a>
        </Tooltip>
      </small>
    );
  ...
  return (
    <>
      {editMode ? (
        ...
        <Tooltip title={t("Alert:Open query in a new tab.")}>
      ...
      {query && !queryResult && (
        <div className="m-t-30">
          <LoadingOutlinedIcon className="m-r-5" />{t("Alert:Loading query data")}
        </div>
      )}
    </>
  );

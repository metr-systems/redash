  ...
  const onEnabledChange = value => {
    if (value || !hasContent) {
      ...
      Modal.confirm({
        ...
        content: t("Alert:Switching to default template will discard your custom template."),
        onOk: () => {
          setSubject(null);
          setBody(null);
          setEnabled(value);
  ...
  return (
    <div className="alert-template">
      <Select
        ...
        <Select.Option value={0} label={t("Alert:Use default template")}>
          {t("Alert:Default template")}
        </Select.Option>
        <Select.Option value={1} label={t("Alert:Use custom template")}>
          {t("Alert:Custom template")}
        </Select.Option>
      </Select>
      {!!enabled && (
        <div className="alert-custom-template" data-test="AlertCustomTemplate">
          <div className="d-flex align-items-center">
            <h5 className="flex-fill">{t("Alert:Subject / Body")}</h5>
            {t("Alert:Preview")}{" "}
          ...
          <HelpTrigger type="ALERT_NOTIF_TEMPLATE_GUIDE" className="f-13">
            <i className="fa fa-question-circle" aria-hidden="true" /> {t("Alert:Formatting guide")}{" "}

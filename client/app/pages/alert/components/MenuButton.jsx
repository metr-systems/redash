...
export default function MenuButton({ doDelete, canEdit, mute, unmute, muted }) {
  ...
  const confirmDelete = useCallback(() => {
    Modal.confirm({
      title: t("Alert:Delete Alert"),
      content: t("Alert:Are you sure you want to delete this alert?"),
      okText: t("Delete"),
      okType: "danger",
      onOk: () => {
        setLoading(true);
    ...
    <Dropdown
      ...
      overlay={
        <Menu>
          <Menu.Item>
            {muted ? (
              <PlainButton onClick={() => execute(unmute)}>{t("Alert:Unmute Notifications")}</PlainButton>
            ) : (
              <PlainButton onClick={() => execute(mute)}>{t("Alert:Mute Notifications")}</PlainButton>
            )}
          </Menu.Item>
          <Menu.Item>

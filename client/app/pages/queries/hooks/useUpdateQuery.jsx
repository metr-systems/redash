...
class SaveQueryConflictError extends SaveQueryError {
  constructor() {
    super(
      i18next.t("Queries:Changes not saved"),
      <React.Fragment>
        <div className="m-b-5">{i18next.t("Queries:It seems like the query has been modified by another user.")}</div>
        <div>{i18next.t("Queries:Please copy/backup your changes and reload this page.")}</div>
      </React.Fragment>
    );
  }
}
...
function confirmOverwrite() {
  return new Promise((resolve, reject) => {
    Modal.confirm({
      title: i18next.t("Queries:Overwrite Query"),
      content: (
        <React.Fragment>
          <div className="m-b-5">{i18next.t("Queries:It seems like the query has been modified by another user.")}</div>
          <div>{i18next.t("Queries:Are you sure you want to overwrite the query with your version?")}</div>
        </React.Fragment>
      ),
      okText: i18next.t("Queries:Overwrite"),
      okType: "danger",
      onOk: () => {
        resolve();
      },
...
function doSaveQuery(data, { canOverwrite = false } = {}) {
  ...
  return Query.save(data).catch((error) => {
    ...
    return Promise.reject(new SaveQueryError(i18next.t("Queries:Query could not be saved")));
  });
}
...
export default function useUpdateQuery(query, onChange) {
  ...
  return useCallback(
    (data = null, { successMessage = i18next.t("Queries:Query saved") } = {}) => {
      if (isObject(data)) {

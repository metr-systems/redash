...
import { OrganizationService } from "../../services/composedDashboard";
...
export default function SubDashboardAssignments({ dashboardId }) {
  ...
  useEffect(() => {
    GlobalDashboard.get(dashboardId).then(setDashboard);
    GlobalDashboard.getAssignments(dashboardId).then(setAssignments);
  ...
  function handleAssign() {
    ...
    setAssigning(true);
    GlobalDashboard.addAssignment(dashboardId, selectedOrgId)
      .then((newAssignment) => {
  ...
  function handleRemove(assignment) {
    ...
    GlobalDashboard.removeAssignment(dashboardId, assignment.assignment_id)
      .then(() => {
  ...
  return (
    <div className="container" style={{ paddingTop: 30, maxWidth: 700 }}>
      <div style={{ marginBottom: 20 }}>
        <a href="/global-api/admin/dashboards" style={{ fontSize: 13 }}>
          ← Back to Dashboards
        </a>
      </div>

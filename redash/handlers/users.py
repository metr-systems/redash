...
class UserResource(BaseResource):
    ...
    def post(self, user_id):  # noqa: C901
        ...
        if email_address_changed and metr_sso.is_enabled():
            # The hand-off matches people by email address, so changing one here
            # detaches that account from the core-backend user it belongs to. The
            # next arrival finds nobody and provisions a second, empty account
            # beside it, and nothing says so.
            abort(
                403,
                message=_(
                    "Email addresses are managed by single sign-on and cannot be "
                    "changed here. Change it in core-backend instead."
                ),
            )

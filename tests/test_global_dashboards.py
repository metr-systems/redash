...
class TemplateDashboardsReadApiTest(GlobalBaseTestCase):
    def setUp(self):
        super().setUp()
    ...
    def _create_template_dashboard(self, org, name="Template A", is_draft=False, is_archived=False):
    ...
    def test_requires_authentication(self):
        ...
        response = anonymous.get("/global-api/template-dashboards")
    ...
    def test_list_returns_only_template_org_dashboards(self):
        ...
        self._create_template_dashboard(template_org, name="In Template")
        self.factory.create_dashboard(org=self.factory.org, name="In Client Org")
        db.session.commit()

        data = self.global_client.get("/global-api/template-dashboards").get_json()
    ...
    def test_list_excludes_drafts_and_archived(self):
        template_org = self._create_template_org()
        self._create_template_dashboard(template_org, name="Published")
        self._create_template_dashboard(template_org, name="Draft", is_draft=True)
        self._create_template_dashboard(template_org, name="Archived", is_archived=True)
        db.session.commit()
    ...
    def test_list_orders_newest_first(self):
        ...
        template_org = self._create_template_org()
        older = self._create_template_dashboard(template_org, name="Older")
        newer = self._create_template_dashboard(template_org, name="Newer")
    ...
    def test_list_paginates(self):
        template_org = self._create_template_org()
        for i in range(3):
            self._create_template_dashboard(template_org, name="D{}".format(i))
        db.session.commit()

        data = self.global_client.get("/global-api/template-dashboards?page=1&page_size=2").get_json()
    ...
    def test_list_empty_when_template_org_absent(self):
        ...
        response = self.global_client.get("/global-api/template-dashboards")
    ...
    def test_result_includes_link_out_url(self):
        template_org = self._create_template_org()
        dashboard = self._create_template_dashboard(template_org, name="Linkable")
        db.session.commit()
